 ...
 ***************************************************
 *
 * FILE NAME  : M1_A1_T1_weather.c      
 * DESCRIPTION: This code retrieves weather data
 *				given by main program, logs the data
 *				into a text file, and prints unto 
 *				LCD if "rain" is detected.
 *
 * *************************************************/
...
{	
	...
	if (output)
	{
		...
		clear_LCD();
		home_LCD();
		LCD_Print ((char*)("Bring UMBRELLA"));
	}
	
	FILE *fp;
	fp = fopen("/home/debian/weatherinfo.txt", "a+");
	fprintf(fp, "%s\n", argv[1]);
	fclose(fp);
}