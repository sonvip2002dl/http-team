 ...
 ***************************************************
 *
 * FILE NAME  : M1_L3_E4b_ADC_sampling.c      
 * DESCRIPTION: Display the ADC value on screen, takes
 *              input from buttons:B3 pressed is increase
 *              and B4 pressed is channel decreased
 *
 ***************************************************/
...
int main(int argc, char** argv)
{
	...
	channel = 0;
	fp = fopen ("adcValue.txt", "w+");	
	int i = 0;
	
    while (1)
	{
    	adcValue = (float) GetMCP3208(channel)*3.3/4096;
    	fprintf(fp, "%f\n", adcValue);
	}
    fclose(fp);
}
