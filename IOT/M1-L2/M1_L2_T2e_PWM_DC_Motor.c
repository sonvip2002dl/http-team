 ...
 ***************************************************
 *
 * FILE NAME  :  M1_L2_T2e_PWM_DC_Motor.c      
 * DESCRIPTION:  Coninually ramps up duty cycle of PWM
 * *************************************************/

#include "mraa_beaglebone_pinmap.h"
#include <signal.h>
int period = 100000000; // 20 ms or 50 Hz cycle
...
int main()
{
	...
	// loop forever ramping the duty cycle 0% to 100%
	while(1)
	{
		for (dty_cycle = 0.0;dty_cycle < 1.0;dty_cycle += 0.1)
		{
			set_duty(dty_cycle);
			sleep(5);
		}
